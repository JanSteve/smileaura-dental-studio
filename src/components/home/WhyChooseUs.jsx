      ...
      <div className="max-w-[1400px] mx-auto px-6">
        ...
        <div className="flex flex-col lg:flex-row gap-16 items-start">
          <ScrollReveal className="lg:w-[40%]">
            <div className="aspect-square rounded-3xl bg-gradient-to-br from-gold-primary/10 via-navy-card to-teal-accent/10 border border-white/5 flex items-center justify-center">
          ...
          <div className="lg:w-[60%] space-y-4">
            {items.map((item, i) => (
              <ScrollReveal key={i} delay={i * 0.1}>
                <div className={`border border-white/5 rounded-xl overflow-hidden transition-colors ${open === i ? 'bg-white/[0.02]' : ''}`}>
                  <button onClick={() => setOpen(open === i ? -1 : i)} className="w-full flex items-center gap-4 px-6 py-5 text-left">
                    ...
                    <span className="font-heading font-semibold text-white text-lg flex-1">{item.title}</span>
