      ...
      <div className="max-w-[1400px] mx-auto px-6">
        ...
        <div className="flex flex-col lg:flex-row gap-12">
          <ScrollReveal className="lg:w-[60%]">
            <div className="relative bg-navy-card rounded-3xl p-10 border border-overlay">
              ...
              <div className="relative z-10">
                ...
                <p className="font-heading font-semibold text-white-soft">{t.name}</p>
            ...
            {testimonials.map((item, i) => (
              <ScrollReveal key={i} delay={i * 0.1}>
                <button onClick={() => setFeatured(i)}
                  className={`w-full text-left glass-card rounded-2xl p-5 transition-all duration-300 ${featured === i ? 'border-gold-primary/40 bg-gold-primary/5' : 'hover:border-overlay-10'}`}>
                  <p className="font-body text-sm text-white-muted line-clamp-2 mb-2">"{item.quote.substring(0, 80)}..."</p>
                  <p className="font-heading text-sm font-semibold text-white-soft">{item.name}</p>
        ...
        <ScrollReveal className="mt-16">
          <div className="overflow-hidden py-4 border-y border-overlay">
            <div className="flex gap-8 animate-[marquee_30s_linear_infinite] whitespace-nowrap">
              {[...Array(6)].map((_, i) => (
                <div key={i} className="flex items-center gap-3 bg-navy-card rounded-full px-6 py-2 border border-overlay flex-shrink-0">
