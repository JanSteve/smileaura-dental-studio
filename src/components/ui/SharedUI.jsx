  ...
  return (
    <div className="rounded-2xl overflow-hidden border border-white/10">
      <div ref={containerRef} className="before-after-slider relative w-full aspect-[4/3]" onMouseDown={() => { isDragging.current = true; }} onTouchStart={() => { isDragging.current = true; }}>
        <div className="absolute inset-0 bg-gradient-to-br from-white/20 to-white/5 flex items-center justify-center"><span className="font-mono text-4xl text-white/30">AFTER</span></div>
        <div className="before-clip absolute inset-0 bg-gradient-to-br from-navy-mid to-navy-card flex items-center justify-center" style={{ clipPath: 'inset(0 50% 0 0)' }}><span className="font-mono text-4xl text-white/20">BEFORE</span></div>
        ...
        <span className="absolute bottom-3 left-3 font-mono text-[11px] text-white/60 z-10">BEFORE</span>
        <span className="absolute bottom-3 right-3 font-mono text-[11px] text-gold-primary z-10">AFTER</span>
      </div>
      {treatment && (<div className="p-4 bg-navy-card"><p className="font-heading font-semibold text-white text-sm">{treatment}</p>{timeframe && <span className="font-mono text-[11px] text-gold-primary">{timeframe}</span>}</div>)}
    </div>
  );
}
    ...
    <div className="space-y-3">
      {items.map((item, i) => (
        <div key={i} className="border border-white/5 rounded-xl overflow-hidden">
          <button onClick={() => setOpenIdx(openIdx === i ? null : i)} className="w-full flex items-center justify-between px-6 py-4 text-left hover:bg-white/[0.02] transition-colors">
            <span className="font-heading font-semibold text-white text-[15px] pr-4">{item.q}</span>
...
export function WhatsAppFloat() {
  return (
    <a href="https://wa.me/919876543210" target="_blank" rel="noopener noreferrer" className="fixed bottom-6 right-6 z-[9999] group" aria-label="Chat on WhatsApp">
      ...
      <div className="absolute bottom-full right-0 mb-2 px-3 py-1.5 bg-navy-card rounded-lg font-mono text-[11px] text-white opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap border border-white/10">Chat with us</div>
    </a>
  );
}
